...
if ( cr_get_option( 'post_sidebars_option' ) == 'global' ) {
    ...
    if ( is_array( $sidebar_exist ) ) {
	$sidebar_exist = array_shift( $sidebar_exist );
	switch ( $sidebar_exist ) {
	    case 'none':
		$sidebar_exist = 'span3';
		break;
	    case 'left':
...
<div class="blog1">
	...
	<!-- Post Date -->
	<?php
	$monthDay = get_the_date( 'd M' );
	$year = get_the_date( 'Y' );
	?>
	<div class="postDate">
	    <div class="dayMonth">
			<p><?php echo esc_html( $monthDay ); ?></p>
	    </div>
	    <div class="year">
	...
	<div class="postDetails">
		...
		<div class="postViews">
			...
			<p class="color">
			    <?php echo esc_html($offer_meta['price']); ?>
			</p>
		</div>
		<div class="postViews">
			<span class="icon-calendar"></span>
			<p class="color">
			    <?php echo esc_html($offer_meta['startDate']); ?>
			</p>
		</div>
		<div class="postViews">
		...
		<?php if ( cr_get_option( 'post_comments_count', '' ) == 1 ) { ?>
    		<a href="#" class="postComment"><span class="icon-comment"></span><p><?php echo get_comments_number( $post->ID ) ?> Comments</p></a>
		<?php } ?>
	    </div>
	    <!-- Post Image -->
	    <?php if ( cr_get_option( 'post_view_image', '' ) == 1 ) { ?>
    	    <div class="postImage">
