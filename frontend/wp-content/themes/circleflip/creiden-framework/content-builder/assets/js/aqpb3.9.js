...
jQuery(document).ready(function($){
	...
	update_block_number();
	var google_fonts = '';
	if (global_creiden.google_fonts) {
		
	for (var i=0; i < global_creiden.google_fonts.length; i++) {
	  google_fonts += global_creiden.google_fonts[i] + ' (Google Fonts) =' + global_creiden.google_fonts[i] + ';';
	};
	}
	function checkStack() {
		idStack = new Array;
        ...
        $( $( this ).attr( 'href' ) )
                .stackableModal()
                 .one( 'show.bs.modal', function() {
                    ...
                    tinyMCE.PluginManager.load('colorpicker_cf', global_creiden.template_dir+'/creiden-framework/content-builder/assets/js/colorpicker/colorpicker.js');
                	tinyMCE.execCommand( 'mceRemoveEditor', true, 'aq_block_' + $this.attr( 'id' ).substring( 15 ) );
						...
						jQuery('.aq_block_' + $this.attr( 'id' ).substring( 15 )+'_tabs_editor_tabbed').each(function(index,element) {
							// if(tinyMCE.get( $(element).attr('id') ))
								...
								if(tinyMCE.get($(element).attr('id'))) {
    								...
    								menubar: false,
    								relative_urls: false,
									content_css : global_creiden.template_dir+"/css/fonts/fonts-admin.php",
									font_formats : 	"Andale Mono=andale mono,times;"+
													...
													"Webdings=webdings;"+
													"Wingdings=wingdings,zapf dingbats;"+google_fonts,
									...
									toolbar1 : "save newdocument | bold italic underline strikethrough | justifyleft justifycenter justifyright justifyfull | styleselect formatselect fontselect fontsizeselect  | wp_adv",
                ...
                .one( 'shown.bs.modal', function() {
                   ...
                   // if(!global_creiden.wordpress_version){
                    ...
                    _.delay(function(){
                        tinyMCE.execCommand( 'mceAddEditor', true, 'aq_block_' + $this.attr( 'id' ).substring( 15 ) );
                        setTimeout(function() {
							if(tinyMCE.get('aq_block_' + $this.attr( 'id' ).substring( 15 ))) {
	...
	var steps = new Array(25,33.3333,50,66.66667,75,100);
	...
	var block_archive,
		...
		resizable_args_Home = {
			...
			resize: function(event, ui) {
			    ui.helper.css("height", "inherit");
			    /*
				var block_width = ui.element.width();
				var ui_width = ui.size.width/block_width*100;
			    ...
			    var ui_size = ui.size.width/$('#blocks-to-edit').width()*100;
			    if(ui_size <= steps[0]) {
		            $(this).css('width',steps[0]+'%');
		            current_step = steps[0];
		        } else if(ui_size >= steps[0] && ui_size <= steps[1]) {
		            $(this).css('width',steps[1]+'%');
		            current_step = steps[1];
		        } else if(ui_size >= steps[1] && ui_size <= steps[2]) {
		            $(this).css('width',steps[2]+'%');
		            current_step = steps[2];
		        } else if(ui_size >= steps[2] && ui_size <= steps[3]) {
		            $(this).css('width',steps[3]+'%');
		            current_step = steps[3];
		        } else if(ui_size >= steps[3] && ui_size <= steps[4]) {
		            $(this).css('width',steps[4]+'%');
		            current_step = steps[4];
		        } else {
		            $(this).css('width',steps[5]+'%');
		            current_step = steps[5];
		        }
			},
			stop: function(event, ui) {
	...
	$(document).on('click', '.block-control-actions a', function() {
		...
		if($clicked.hasClass('delete')) {
			...
			$.ajax({
				...
				type: "POST",
				data : {
					exportedData : $clicked.parents('li.block').find('input,textarea,select').serialize(),
	...
	$('.deleteTemplates').click(function(e) {
		...
		$('#retrievePosts').click(function(e) {
			...
			$.ajax({
				...
				type: 'post',
				data: {
						pageBlocks:  $('#blocks-to-edit').parents('form').serialize(),
		...
		$('#publish').click(function(e) {
			$('#publishing-action .spinner').show().css('visibility','visible');
			if($("#post_type").val() == 'page') {
				if($("#importBuilderTemplate").val() !== '') {
					...
					// Overriding the activate revisions for future update
					if(0) {
						if (importing_ajax_done == true) {
							importing_ajax_done = false; // reset flag
							return; // let the event bubble away
						...
						$.ajax({
							...
							type: "POST",
							data : {
								history : $('#blocks-to-edit').parents('form').serialize(),
								postID  : $("#post_ID").val(),
	...
	function handleSize(element) {
		...
		$('.importCertainBlock').click(function() {
			$(this).find('div').show();
		});
		$('#crdn-importCertainBlock button').click(function() {
			$(this).find('div').show();
			var currentBlocks = $("#blocks-to-edit").html();
			$("#blocks-to-edit").html('');
			$.ajax({
				url: global_creiden.ajax_url,
				type: "POST",
				...
				success: function(data) {
					crdn.undo.prototype.setStorage();
					$("#blocks-to-edit").append(data).prepend(currentBlocks);
...
jQuery(window).load(function() {
	jQuery("#post").on('submit',function(event) {
		...
		jQuery('[name*="aq_blocks[aq_block___i__]"]').attr('disabled','disabled');
		//this.submit();
	});
