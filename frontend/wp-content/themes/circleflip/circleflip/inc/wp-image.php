...
function circleflip_register_retina() {
...
add_action('wp_head', 'circleflip_register_retina');

function circleflip_check_image_size($size = 'thumbnail') {
...
function circleflip_image_downsize($ignore = false , $id = 0 , $size = 'medium') {
	...
	if ( !wp_attachment_is_image($id) )
	        ...
	        $new_img_meta  =  circleflip_check_image_size($size) ;
	       if(!$new_img_meta){
	       		return array($img_url, $meta['width'], $meta['height'],false);
	       }
	...
	function circleflip_add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	        global $_code125_image_sizes;
	        $_code125_image_sizes[$name] = array( 'width' => absint( $width ), 'height' => absint( $height ), 'crop' => (bool) $crop );
	}


	add_filter('image_downsize','circleflip_image_downsize', 99, 3);
?>