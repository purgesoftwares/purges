<?php
/* Aqua Tabs Block */
if(!class_exists('CR_Features_Home')) {
	class CR_Features_Home extends AQ_Block {
		...
		function __construct() {
			$block_options = array(
				...
				'tab'    => 'Content',
                                ...
                                'desc' => 'Adds iconed text snippets.'
			);
				...
				<p class="description">
					<span class="leftHalf">
						...
						<span class="description_text">
							Choose titles color, this will apply only for layout 1 and 2  
						</span>
					</span>
					<span class="rightHalf">
						<?php 
						echo circleflip_field_color_picker('title_color', $block_id, $title_color, $defaults['title_color']) ?>
					</span>
				</p>
				<p class="description">
					<span class="leftHalf">
				...
				<div class="sortable-body">
					...
					<p class="tab-desc description">
						...
						<span class="rightHalf">
							...
							<select id="<?php echo esc_attr( $this->get_field_id('tabs') ) ?>-<?php echo esc_attr($count) ?>-choose_type"
								...
								<?php foreach( $choose_type as $key=>$value) { ?>
									<option value="<?php echo esc_attr($key) ?>" '<?php echo selected( (isset($tab['choose_type']))?$tab['choose_type'] : '', $key, false ) ?>'><?php echo esc_url($value) ; ?></option>
								<?php } ?>
							</select>
							</span>
						</span>
					...
					<div class="description half last">
						<span class="leftHalf">
							...
							<span class="description_text">
								Choose Icon Color, This Will Apply only for layout 1
							</span>
						</span>
						<span class="rightHalf">
					...
					if($offer != 'offer'){
						foreach( $tabs as $tab ){
							...
							if($tab['choose_type'] == 1){
								$imageIcon = wp_get_attachment_image($tab['imagesrc'], 'icon_box_image');
								$image_class = 'image_added';
								$image_class_container = 'imageIconBox';
							}else if($tab['choose_type'] == 0) {
								$imageIcon = '<span class="'.$tab['icon'].'" style="color:'.esc_attr($style_color).'"></span>';
								$image_class = '';
								$image_class_container= '';
							}
								$output .= '
								<li class="'. esc_attr($layout .' grid2 animateCr '.$entrance_animation).'">';
									if($tab['link'] != '') $output .= '<a href="'.esc_url($tab['link']).'">';
										$output .= '<div class="image_title_wrap">';
											if ( isset( $imageIcon ) ) {
												$output .= '<div class="featureHomeImage '.esc_attr($image_class.' '.$feature2).'">'.$imageIcon.'</div>';
											}
											$output .= '<h3 class="featureHomeTitle" style="color:'.esc_attr($title_color).'">'.$tab['title'].'</h3>
										</div>';
									if($tab['link'] != '') $output .= '</a>';
									$output .= '<div class="featureHomeText">'.wpautop(do_shortcode(htmlspecialchars_decode($tab['content']))).'</div>
								</li>
								';
						}
					}else{
