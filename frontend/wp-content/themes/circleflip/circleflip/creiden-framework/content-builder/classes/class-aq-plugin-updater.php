	...
	class AQ_Plugin_Updater {
		...
		function plugin_api_call($defaults, $action, $args) {
			...
			if (is_wp_error($request)) {
				$result = new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'circleflip-builder'), $request->get_error_message());
			} else {
				$result = unserialize($request['body']);
				if ($result === false) {
					$result = new WP_Error('plugins_api_failed', __('An unknown error occurred','circleflip-builder'), $request['body']);
				}
			}
			return $result;
		}
		...
		function upgrader_post_install( $true, $hook_extra, $result ) {
			...
			if( isset($hook_extra['plugin']) && stristr($hook_extra['plugin'], $this->args['slug']) ) {
				...
				echo '<p>' . __('Correcting plugin folder name & activating plugin...', 'circleflip-builder') .'</p>';
				...
				$fail		= __('The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'circleflip-builder');
				$success	= __('Plugin reactivated successfully.', 'circleflip-builder');
